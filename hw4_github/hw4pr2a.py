...
# 	nll_sum = -sum(temp) + reg / 2 * np.linalg.norm(W) ** 2
# 	nll = nll_sum.item(0)
...
def get_description(X, y, W):
	...
	"*** YOUR CODE HERE ***"
	# started from the solution here
	#got from solution cuz there was a bug with my code
	m, n = X.shape
	y_pred = predict(X, W)
...
def plot_description(X_train, y_train, X_test, y_test):
	...
	"""
	reg_list = reg_list = [0., 0.01, 0.1, 0.5, 1.0, 2.0, 5.0, 10.0, 20.0, 30.0]
	reg_list.sort()
	a_list = []
	p_list = []
	r_list = []
	...
	#	   obtained, and append those into the corresponding list
		#copy the solution to understand
	"*** YOUR CODE HERE ***"
